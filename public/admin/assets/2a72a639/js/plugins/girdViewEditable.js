function editableGridview(gridId) {
    ...
    this.drawEditor = function (el) {
        ...
        if (el.attr('class') != 'editColumn' || el.children('.inplace-editor').length > 0) {
            return false;
        } //|
        wd = el.width();
        txt = el.text();
        ...
        switch (el.data('widget')) {
            case 'colpick':
                ...
                $(input).colpick({
                    layout: 'hex', //submit:0,
                    color: colour, //colorScheme:'light',
    ...
    this.updateDb = function (el) {
        ...
        if (data == undefined) {
            data = el.html();
        } // this should catch colour fields
        if (data == undefined) {
        ...
        if (typeof this.otherPostData == 'object') {
            for (var attrname in this.otherPostData) {
                postData[attrname] = this.otherPostData[attrname];
            }
        }
        if (this.includeKey) {
        ...
        $.post(url, $.param(postData), function (data) {
            if (data.status || data.status == 'true' || data.status == 'ok') {
    ...
    this.addRowSave = function (trow) {
        ...
        $.ajax({
            url: this.createUrl, data: data, type: 'POST'
        })
