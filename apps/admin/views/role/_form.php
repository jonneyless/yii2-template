    ...
    <div id="tab-auth" class="tab-pane">
        <div class="panel-body">
            ...
            <table class="table table-bordered table-responsive table-auth">
                ...
                <?php $auths = \admin\models\AdminAuth::find()->where(['parent' => ''])->all(); ?>
                <?php foreach($auths as $auth){ ?>
                    <?php $checkParant = in_array($auth->key, $model->authArr); ?>
                    <tr>
                        <th>
                            <label for="auth-<?= $auth->key ?>">
                                <input id="auth-<?= $auth->key ?>" type="checkbox" name="<?= $inputName ?>[]"<?php if($checkParant){ ?> checked<?php } ?> value="<?= $auth->key ?>" /> <?= $auth->name ?>
                            </label>
                        </th>
                        <td>
